...
object PublicSavesEndpoints {
  ...
  val uploadPublicText: PublicEndpoint[Text, Unit, String, Any] =
    endpoint.post
      .in("uploadPublicText")
      .in(jsonBody[Text])
      .out(stringBody)
  ...
  val getPublicFile: PublicEndpoint[String, ApiError, (InputStream, String), Any] =
    endpoint.get
      .in("file" / path[String]("publicToken"))
      .errorOut(oneOfApiError)
      .out(inputStreamBody)
